import { Mesh, Vector3, SplineCurve, BufferGeometry } from "three";
...
export default class AnimatedMeshLine extends Mesh {
  constructor({
    ...
    if (!points) {
      ...
      linePoints = new BufferGeometry().setFromPoints(points);
    }
  ...
  update() {
    ...
    if (this.isDying()) {
      this.material.uniforms.opacity.value =
        0.9 + (this.material.uniforms.dashOffset.value + 1) / this.dashLength;
    }
  }
